...
def wrapper_expand_boxes(raw_annotation_file, margins):
    ...
    for key, item in raw_annotation_data.items():
        ...
        for idx, annotation in enumerate(raw_annotations):
            final_annotations = {}
            final_annotations['text'] = annotation["text"]
...
def main():
    ...
    parser.add_argument(
        ...
        default=4,
        help="number of CPU workers -- multiprocessing"
    )
    # , nargs='+', type=int
    # parse the arguments
    ...
    print(f"dir_base_input_folder: {dir_base_input_folder}")
    print(f"margins: {margins}")
    print(f"No of workers -- in multiprocessing: {workers}")
    ...
    jobs = raw_annotation_files

    result = process_map(partial(worker, **kwargs), jobs, max_workers=workers, chunksize=1)
